...
impl PN {
    const fn new(p: Vec2, n: Vec2) -> Self {
        Self { p, n }
    }
}
...
fn stroke_path(path: &[PN], closed: bool, width: f32, feather: f32, out: &mut GeomSink) {
    ...
    if closed {
        let base = base_vertex + (path.len() * 2) as u32;
...
fn flatten_rrect(shape: RRect, out: &mut Vec<PN>) {
    let rect = shape.rect;
    let r = shape.radius;
    let corner_centers = [
    ...
    if r > 0.0 {
        ...
        for (icorner, center) in corner_centers.iter().enumerate() {
            for i in 0..=nseg {
                ...
                let p = center + dir * r;
                out.push(PN::new(p, dir));
                angle = PI * (1.0 - 0.5 * (icorner as f32 + i as f32 / nseg as f32));
            }
        }
    } else {
        // No radius, just push the corners
        out.push(PN::new(vec2(rect.min.x, rect.min.y), vec2(-1.0, -1.0)));
        out.push(PN::new(vec2(rect.max.x, rect.min.y), vec2(1.0, -1.0)));
        out.push(PN::new(vec2(rect.max.x, rect.max.y), vec2(1.0, 1.0)));
        out.push(PN::new(vec2(rect.min.x, rect.max.y), vec2(-1.0, 1.0)));
    }
}
#[derive(Default)]
...
impl Tessellator {
    ...
    pub fn fill_rrect(&mut self, rect: RRect) {
        let mut p = Vec::new();
        flatten_rrect(rect, &mut p);
        tess_feathered_polygon(&p, 0.5, 0.5, &mut self.geometry);
    }

    pub fn stroke_rrect(&mut self, rect: RRect, width: f32) {
        let mut p = Vec::new();
        flatten_rrect(rect, &mut p);
    ...
    pub fn stroke_line(&mut self, p0: Vec2, p1: Vec2, width: f32) {
        ...
        let path = [PN::new(p0, n), PN::new(p1, n)];
        stroke_path(&path, false, width, 0.5, &mut self.geometry);
    }

    pub fn stroke_path(&mut self, path: PathSlice, width: f32) {
        ...
        for seg in path.iter() {
            match seg {
                ...
                PathSegment::LineTo(p1) => {
                    self.stroke_line(cur_pos, p1, width);
                    cur_pos = p1;
                }
                _ => {
                    // TODO
