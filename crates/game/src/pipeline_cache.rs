...
fn create_graphics_pipeline_from_archive(archive: &PipelineArchive, name: &str) -> Result<gpu::GraphicsPipeline, PipelineCreateError> {
    let entry = archive.find_graphics_pipeline(name).ok_or_else(|| PipelineCreateError::PipelineNotFound(name.to_string()))?;
    ...
    let gpci = gpu::GraphicsPipelineCreateInfo {
        set_layouts: &[],
        push_constants_size: dbg!(entry.push_constants_size) as usize,
...
fn create_compute_pipeline_from_archive(archive: &PipelineArchive, name: &str) -> Result<gpu::ComputePipeline, PipelineCreateError> {
    let entry = archive.find_compute_pipeline(name).ok_or_else(|| PipelineCreateError::PipelineNotFound(name.to_string()))?;
